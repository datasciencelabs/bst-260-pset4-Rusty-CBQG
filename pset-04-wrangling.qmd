...
population <- population |> 
  ...
  clean_names() |> 
  as_tibble() |> 
  rename(state_name = name, fips = state) |> 
  mutate(across(c(pop_2020, pop_2021), as.numeric)) |> 
  select(-fips) |> 
  left_join(tibble(state_name = c(state.name, 
...
Ans: The reason why only 1000 rows is that the API defaults to 1000 limits.
